...
import sys
import os
from word2vec import Word2Vec, Sent2Vec, DirectoryForSentence, LineSentence

logging.basicConfig(format='%(asctime)s : %(threadName)s : %(levelname)s : %(message)s', level=logging.INFO)
logging.info("running %s" % " ".join(sys.argv))

input_file = 'E:\\Allen\\R\\emnlp2015\\word2vec\\test.txt'
model = Word2Vec(LineSentence(input_file), size=100, window=5, sg=0, min_count=5, workers=30) #min_count = 50
